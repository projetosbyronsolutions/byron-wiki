...
Uma variável é um container para um valor, como um número que podemos usar em uma operação de adição, ou uma sequência de texto que possamos usar como parte de uma frase.
...
Isso indica que a linha de códig será iniciada com as palavras `let` ou `var`, seguida pelo **identificador**, um **sinal de igual** e um **valor** ou **expressão**.

**identificador:** nome da variável  
&nbsp;
**valor ou expressão:** valor inicial que será armazenado dentro daquela variável.
&nbsp;
...
Nesse caso, as variáveis contêm valores dentro. Ou seja, a variável `nome` contém o valor `'Bianca'` e a variável `faculdade` contém valor `'Unifei'`.
...
Camelcase é uma convenção de nomenclatura de variáveis utilizada em algumas linguagens de programação, como o próprio JavaScript. Ela consiste em escrever cada palavra no nome de uma variável com a letra inicial maiúscula, com exceção da primeira palavra da frase.
...
Caso queira exibir apenas um número no terminal, dê o console.log da forma mostrada a seguir. O número entre colchetes, no caso, o 0, indica a posição do elemento no vetor. Logo, será exibido o primeiro elemento do vetor.
