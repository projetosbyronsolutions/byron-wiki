...
async function obterDadosUsuario() {
	try {
		const resposta = await fetch(
			'https://api.github.com/users/bianca-rossi'
		);
		if (!resposta.ok) {
...
<Quiz
	question="1. Qual é a finalidade do método .then() em uma chamada Axios?"
	answers={[
...
<Quiz
	question="2. O que esse código faz?"
	answers={[
...
<Quiz
	question="3. O que o método .json() faz em uma resposta fetch?"
	answers={[
...
<Quiz
	question="4. Como você pode lidar com erros ao fazer uma requisição com fetch?"
	answers={[
...
<Quiz
	question="5. Como você lidaria com erros ao usar async/await?"
	answers={[
...
<Quiz
	question="6. O que a palavra-chave await faz em uma função assíncrona?"
	answers={[
...
<Quiz
	question="7. Qual é o propósito do bloco try...catch dentro da função processarDados() no código acima?"
	answers={[
